...
while(!feof(fp))
	...
	if (!memcmp(str,"]title",strlen("]title"))) {fprintf(fo,"<h3>[%i] %s</h3>\n",message_counter++,str+strlen("]title"));}
		...
		if(++messages_processed>(MSG_ON_PAGE-1))
			...
			page++;
			// вывод ссылки "назад"
			fprintf(fo,"<br><a href=\"page%i.html\">&lt; Назад</a>\n", page);
			if (page!=1) fprintf(fo," <a href=\"/\">Home</a>");
			if (messages/MSG_ON_PAGE)
				fprintf(fo," <a href=\"page%i.html\">End</a>\n", messages/MSG_ON_PAGE);
			
			switch (page)
				{
				case 1:
				fprintf(fo,
				"<br><hr>Generated by <a href=\"https://github.com/prool/proolcms\">ProolCMS</a>, compile date %s %s <br> Generate date %s, messages %i\n",
				__DATE__, __TIME__, ptime(), messages);
				break;
				case 2:
				fprintf(fo," <a href=\"/\">Вперед &gt;</a>\n");
				break;
				default:
				fprintf(fo," <a href=\"page%i.html\">Вперед &gt;</a>\n", page-1);
				}
